...
function LandingSite(props: { setShowLandingSite: Function, gameMode: number, setGameMode: Function }) {
...
function InstructionSite(props: { setShowWelcomeSite: Function, gameMode: number }) {
    const scenariosData: scenarios[] = useJson(3);
            ...
            <div className="text-center mt-24 lg:mt-56 ">
                ...
                <p className="text-xl lg:text-3xl font-light px-6"
                    dangerouslySetInnerHTML={{
                        __html: scenariosData[props.gameMode].howToPlay,
                    }}
                ></p>
            </div>
...
function WelcomeSite(props: { setShowWelcomeSite: Function, gameMode: number, setGameMode: Function }) {
    const [showLandingSite, setShowLandingSite] = useState(true);

    return (
        <>
            {showLandingSite && <LandingSite setShowLandingSite={setShowLandingSite} gameMode={props.gameMode} setGameMode={props.setGameMode} />}
            {!showLandingSite && <InstructionSite setShowWelcomeSite={props.setShowWelcomeSite} gameMode={props.gameMode} />}
        </>
    );
}
